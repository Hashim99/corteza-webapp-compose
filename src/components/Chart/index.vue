<template>
  <div class="position-relative h-100 w-100">
   <!--  <canvas ref="chartCanvas" class="p-2 mh-100 w-auto m-auto" /> -->

    <iframe
    class="mh-150 w-auto ml-5"
    ...
    async updateChart() {
      ...
      try {
        ...
        if (!!data.labels && Array.isArray(data.labels)) {
          ...
          if (module) {
            field = [...module.fields, ...module.systemFields()].find(
              ({ name }) => name === field
            );
          }
          ...
          if (field && ["User", "Record"].includes(field.kind)) {
            if (field.kind === "User") {
              ...
              if (recordModule && data.labels) {
                await Promise.all(
                  data.labels.map(recordID => {
                    if (recordID && recordID !== "undefined") {
                      return this.$ComposeAPI
                        .recordRead({
                          namespaceID,
                          moduleID: recordModule.moduleID,
                          recordID
                        })
                        .then(record => {
                          ...
                          if (field.options.recordLabelField) {
                            // Get actual field
                            const relatedField = recordModule.fields.find(
                              ({ name }) => name === field.options.labelField
                            );

                            return this.$ComposeAPI
                              .recordRead({
                                namespaceID,
                                moduleID: relatedField.options.moduleID,
                                recordID:
                                  record.values[field.options.labelField]
                              })
                              .then(labelRecord => {
                                record.values[field.options.labelField] = (
                                  labelRecord.values.find(
                                    ({ name }) =>
                                      name ===
                                      this.field.options.recordLabelField
                                  ) || {}
                                ).value;
                                return record;
                              });
                  ...
                  data.labels = records.map(record => {
                    const value = field.options.labelField
                      ? record.values[field.options.labelField]
                      : record.recordID;
          ...
          data.labels = data.labels.map(l =>
            l === "undefined" ? this.$t("chart:undefined") : l
          );
          ...
          if (canvas) {
            const newRenderer = () =>
              new ChartJS(this.$refs.chartCanvas.getContext("2d"), {
                options,
                plugins: [...plugins, Funnel, Gauge, csc],
                data,
                type
              });
            if (this.renderer) {
